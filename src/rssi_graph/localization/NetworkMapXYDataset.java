 ...
 * @author ph4r05
 */
public class NetworkMapXYDataset extends AbstractXYDataset
        implements XYDataset, PublicCloneable {

    public static final int ADDMODE_APPEND=1;
    public static final int ADDMODE_REMOVE=2;
     ...
     * @return 
     */
    public double[][] getSeriesData(int series){
        double[][] seriesData = (double[][]) this.seriesList.get(series);
        return seriesData;
    }
    ...
    public void addSeries(Comparable seriesKey, double[][] data) {
        this.addSeries(seriesKey, data, ADDMODE_REMOVE);
    }
     ...
     *     second containing the y-values).
     */
    public void addSeries(Comparable seriesKey, double[][] data, int addMode) {
        if (seriesKey == null) {
        ...
        if (seriesIndex == -1 || addMode == ADDMODE_APPEND) {  // add a new series
            if (seriesIndex == -1){
                this.seriesKeys.add(seriesKey);
                this.seriesList.add(data);
            } else {
                // exists, need to append
                double[][] seriesData = this.getSeriesData(seriesIndex);
                double[][] newSeriesData = new double[2][seriesData[0].length + data[0].length];
                
                for(int ti=0, cnOld=seriesData[0].length; ti<cnOld; ti++){
                    newSeriesData[0][ti] = data[0][ti];
                    newSeriesData[1][ti] = data[1][ti];
                }
                
                for(int ti=0, cnNew=data[0].length, cnOld=seriesData[0].length; ti < cnNew; ti++){
                    newSeriesData[0][ti+cnOld] = data[0][ti];
                    newSeriesData[1][ti+cnOld] = data[1][ti];
                }
                
                this.seriesList.remove(seriesIndex);
                this.seriesList.add(seriesIndex, newSeriesData);
            }
        }
        else if (seriesIndex == -1 || addMode == ADDMODE_REMOVE) {  // replace an existing series
            this.seriesList.remove(seriesIndex);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof NetworkMapXYDataset)) {
            return false;
        }
        NetworkMapXYDataset that = (NetworkMapXYDataset) obj;
    ...
    public Object clone() throws CloneNotSupportedException {
        NetworkMapXYDataset clone = (NetworkMapXYDataset) super.clone();
