/**
 * Copy of DefaultBoxAndWhiskerXYDataset.java from JFreeChart library with
 * misc changes. Main change is converted Xvalue from Date to Integer
 *
 * It would be possible to use old DefaultBoxAndWhiskerXYDataset for same purpose
 * (Fill Data with proper values and by proper way), since when retrieving X value
 * It is converted to (long). But for performance point of view I have altered this
 * class to work with Integer natively.
 * (with Date as Xvalue there could be some display issues) *
 *
 * @author ph4r05
 */
...
public class RSSIBoxAndWhiskerXYDataset
     ...
     */
    public RSSIBoxAndWhiskerXYDataset(Comparable seriesKey) {
     ...
     *
     * @param xvalue  the xvalue (<code>null</code> not permitted).
     * @param item  the item (<code>null</code> not permitted).
     */
    public void add(Double xvalue, BoxAndWhiskerItem item) {
        this.xvalue.add(xvalue);
        this.items.add(item);
    ...
    public Number getX(int series, int item) {
        return new Double((Double)this.xvalue.get(item));
    }
    ...
    public List getOutliers(int series, int item) {
        // ph4r05 edit: do not display outliers in graph.
        // this dataSet won't return any outlier
        if (true) return new LinkedList();
    ...
    public boolean equals(Object obj) {
        ...
        RSSIBoxAndWhiskerXYDataset that
                = (RSSIBoxAndWhiskerXYDataset) obj;
        if (!ObjectUtilities.equal(this.seriesKey, that.seriesKey)) {
            return false;
        }
        if (!this.xvalue.equals(that.xvalue)) {
            return false;
        }
    ...
    public Object clone() throws CloneNotSupportedException {
        RSSIBoxAndWhiskerXYDataset clone
                = (RSSIBoxAndWhiskerXYDataset) super.clone();
        clone.xvalue = new java.util.ArrayList(this.xvalue);
